...
import com.wizarpos.emvsample.keys.KeyInfo;
import com.wizarpos.emvsample.keys.KeyInjector;
import com.wizarpos.emvsample.keys.PINPadException;
import com.wizarpos.emvsample.service.CallHomeService;
...
public class FuncActivity extends AppCompatActivity implements Constants, IFuntionListener
{
    ...
    {
        super.onPause();
        Log.i("okh", "function on pause");
		Intent intent = new Intent(this, CallHomeService.class);
		startService(intent);
    }
    ...
    protected void showPropress(String progressString){
		if(progressBar == null){
			progressBar = new ProgressDialog(this);
		}
		progressBar.setCancelable(false);
		progressBar.setMessage(progressString);
		progressBar.show();
	}
...
//		startActivity(intent);
	...
	public void setTitle(TextView textTitle6){
		if(textTitle6 != null){
			if(appState.balanceEnc){
				textTitle6.setText("Balance");
			}else if(appState.refund){
				textTitle6.setText("Refund");
			}else  if(appState.purchaseWithCashBack){
				textTitle6.setText("Purchase with cashback");
			}else if(appState.revarsal){
				textTitle6.setText("Reversal");
			}
			else{
				textTitle6.setText("Purchase");
			}
		}
...
//	}
    ...
    public byte[] getKeyInfo(String key) throws PINPadException, UnrecoverableKeyException,
            CertificateException, NoSuchAlgorithmException, KeyStoreException, IOException {
        KeyInjector injector = new KeyInjector();
        AuthInfo authInfo = injector.getAuthInfo();

        Logger.info("Got authinfo successfull");
        KeyStore keyStore = KeyStore.getInstance("JKS");
        keyStore.load(getAssets().open("itex.jks") , "Itex4839".toCharArray());
        PublicKey publicKey = keyStore.getCertificate("server").getPublicKey();
        KeyData keyData = new KeyData(2,1, 16, key);
        byte[] singnature = getSign(keyData.getKey(authInfo), authInfo.getRandom());
        KeyInfo keyInfo = new KeyInfo(keyData,publicKey.getEncoded(),authInfo.getRandom(),singnature,authInfo);
        return keyInfo.getKeyInfo();
    }

    private byte[] getSign(byte[] keydata, byte[] rand) throws KeyStoreException,
            IOException, CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException, PINPadException {
        KeyStore keyStore = KeyStore.getInstance("JKS");
        //todo set File input path;
        keyStore.load(getAssets().open("itex.jks") , "Itex4839".toCharArray());
        byte[] res = new byte[0];
        CommonUtils.append(keydata,res);
        CommonUtils.append(rand,res);
