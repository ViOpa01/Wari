...
//            val withdrawaldetails = WithdrawalDetails(wallet = mWalletId, userName = mWalletUsername, password = mPlainPassword, pin = mEncryptedPin, type = "default", amount = amountToDebit, vendorBankCode = mBankCode, channel = "ANDROIDPOS", phone = "", paymentMethod = "card", productCode = mProductCode, reference = clientReferenceKey.generateReference(getBaseContext()), pfm = clientReferenceKey.generatePFM(transactionResult, getBaseContext()))
            Log.d("okh", "${response.amountToDebit} init amount");
            //  intent.putExtra("withdrawaldetails", withdrawaldetails);

//            FuncActivity.appState.accountNumber = "", FuncActivity.appState.accountName ="", FuncActivity.appState.recivingBank=""
            SecureStorage.store("amount", response.amountToDebit)
            ...
            FuncActivity.appState.trans.transAmount =  response.amountToDebit
    ...
    private fun verifyWithdrawalAccountDetails(){
        ...
        try {
            ...
            val accountDetails = AccountLookUpDetailWithdrawal(wallet = mWalletId, username = mWalletUsername, type = "default", password = mWalletPassword, amount = txtAmount.text.toString().toFloat() * 100, channel = "ANDROIDPOS")
                ...
                override fun onResponse(call: Call<LookupSuccessModel>, response: retrofit2.Response<LookupSuccessModel>) {
                    ...
                    if (response.body() != null) {
                        ...
                        if (response.body()!!.status != 1) {
                            ...
                            alert {
                                title = "${response.body()!!.message}"
                                message = "Beneficiary- ${response.body()!!.beneficiaryName}\n \nAmount - N$amount\n \nConvenience fee - N${response.body()!!.convenienceFee.toFloat() / 100}"
                                positiveButton("Continue") {
                                    debitWallet()
                                }
...
//                                           Log.d("vasDetails >>>>>>",Gson().toJson(vasDetails))
