...
//            it?.let(this::showResultScreen)
           Log.d("FAct","Print")
            ...
            val error = true
            val cabletvModel: Models.CableTvModel? = Models.CableTvModel(error=payResponse.error,iuc = payResponse.smartCardCode)
...
//				String vasTerminalId = SecureStorage.retrieve(Helper.,"");
            ...
            val vasDetails = Models.VasDetails(cref,amount, wallet, vasmerchantName, merchantID, merchantName, merchantTerminalId, product, responsemessage, vasmerchantID, transactionRef, VasServices.CASH, logo, date, error, Models.CABLE_TV, cabletvModel!!)
...
//        val smartCardNumber = smartcardnumber.text.toString()
        ...
        val emvCard = EmvCard(FuncActivity.appState.trans.cardHolderName, FuncActivity.appState.trans.track2Data, FuncActivity.appState.trans.iccData, pinInfo)
            ...
            val pfm = com.itex.richard.payviceconnect.model.Pfm(PfmStateGenerator(this).generateState(), PfmJournalGenerator(FuncActivity.appState.trans.transactionResult, FuncActivity.appState.nibssData.configData, false, amount.toString(), emvCard, "Startimes",bouquet , "").generateJournal())
