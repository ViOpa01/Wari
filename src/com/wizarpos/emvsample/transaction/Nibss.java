...
//    poslibdb= (getA).poslibdb
    ...
    private static class saveVasKeyHolder extends AsyncTask<VasTerminalData, Integer, Void> {
        protected Void doInBackground(VasTerminalData...vasTerminalData) {
            Log.d("Before Saving ",new Gson().toJson(vasTerminalData[0]));

            SecureStorage.store(Helper.VAS_COMMUNICATOR,  vasTerminalData[0]);


            SecureStorage.store(Helper.MID,  vasTerminalData[0].getMid());
...
//        hostInteractor.getTransactionResult(transactionType,connectionData,transactionData,vasTerminalDetails.getSessionKey(),vasTerminalDetails.getPinKey())
    ...
    //Used for Vas
    public void vasGoOnline(EmvCard emvCard, final Host.TransactionType transactionType,
                         final InputData inputData, KeyHolder keyHolder, ConfigData configData,
                         ConnectionData connectionData, final Nibs<TransactionResult> resultNibs){
        TransactionData transactionData = new TransactionData(inputData,emvCard, configData, keyHolder);
...
//    }
    ...
    private Single<VasTerminalData> getVasTerminalService(String tid)  {
...
//        return VasTerminalService.Factory.INSTANCE.getService().getVasTerminalDetails();
    ...
    private void getVasKeys(String tid )
    {
